...
//		]
	...
	val validationObjectives = new ConstraintsObjective("validProcess").withHardConstraint(
		initNodeWithInvalidNumberOfControlOut, ModelQueryType::NO_MATCH).withHardConstraint(initNodeWithControlIn,
		ModelQueryType::NO_MATCH).withHardConstraint(finalNodeWithInvalidNumberOfIns, ModelQueryType::NO_MATCH).
		withHardConstraint(finalNodeWithControlOut, ModelQueryType::NO_MATCH).withHardConstraint(
			forkNodeWithInvalidNumberOfIns, ModelQueryType::NO_MATCH).withHardConstraint(
			forkNodeWithInvalidNumberOfOuts, ModelQueryType::NO_MATCH).withHardConstraint(
			joinNodeWithInvalidNumberOfIns, ModelQueryType::NO_MATCH).withHardConstraint(
			joinNodeWithInvalidNumberOfOuts, ModelQueryType::NO_MATCH).withHardConstraint(
			decisionNodeWithInvalidNumberOfIns, ModelQueryType::NO_MATCH).withHardConstraint(
			...
			controlFlowWithInvalidNumberOfControlTo, ModelQueryType::NO_MATCH)//		.withHardConstraint(redundantControlFlows,ModelQueryType::NO_MATCH)
		.withHardConstraint(finalNotReachableFromNode, ModelQueryType::NO_MATCH).withHardConstraint(
			initDoesNotReachNode, ModelQueryType::NO_MATCH).withLevel(0)
}
